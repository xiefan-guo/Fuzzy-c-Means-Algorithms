	...
	while (True):
		...
		for j in range(0, cluster_number):
			...
			for i in range(0, len(data[0])):
				...
				for k in range(0, len(data)):
					# 分子
					dummy_sum_num += (U[k][j] * data[k][i] * w[j] * distance_matrix[k][j])
					# 分母
					dummy_sum_dum += (U[k][j] * distance_matrix[k][j] * w[j])
				# 第i列的聚类中心
				current_cluster_center.append(dummy_sum_num/dummy_sum_dum)
			# 第j簇的所有聚类中心
			C.append(current_cluster_center)
...
if __name__ == '__main__':
	...
	start = time.time()
	# 调用模糊C均值函数
	first_U, first_V = wfcm(data, 3, 2, w, [])
	#print("-----------------------------------------")
	# 剩余9份的迭代
	sum_V, sum_w = ilteral_all(ret, ns, first_U, first_V, w, 3)
	# print("sum_V:")
	# print_matrix(sum_V)
	# print("sum_w:")
	# print_matrix(sum_w)
    # 最终的wfcm
	final_U, final_V = wfcm(sum_V, 3, 2, sum_w, []) 
	# 加载完整数据
